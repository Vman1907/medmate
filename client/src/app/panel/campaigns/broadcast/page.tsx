...
import { TemplateMessageWithoutForwardProps } from '../../conversations/_components/molecules/message-input';
...
export default function BroadcastPage() {
	...
	const handleTemplateSave = (template: TemplateMessageWithoutForwardProps) => {
	...
	const handleWarningTemplateSave = (template: TemplateMessageWithoutForwardProps) => {
		form.setValue('warning.template_id', template.template_id);
		form.setValue('warning.template_name', template.template_name);
		form.setValue('warning.template_body', template.body);
		form.setValue('warning.template_carousel', template.carousel);
		form.setValue('warning.template_buttons', template.buttons);
		form.setValue('warning.template_header', template.header);
	};
	...
	function scheduleCampaign(forceSchedule: boolean) {
		...
		if (data.warning?.numbers.length === 0 || !data.warning?.template_id) {
			delete data.warning;
		}

		toast.promise(scheduleBroadcast({ ...data, forceSchedule }), {
	...
	const isValid = broadcastSchema.safeParse(fields);
		...
		<div className='flex flex-col gap-4 justify-center p-4'>
			...
			<Form {...form}>
				<form className='w-full space-y-2'>
							...
							<FormItem className='space-y-0 flex-1 max-w-sm'>
								...
								<FormControl>
									<TemplateDialog
										...
										onConfirm={(details) =>
											handleTemplateSave(details as unknown as TemplateMessageWithoutForwardProps)
										}
									>
					...
					<Separator />
					<div className='border-2 p-4 rounded-lg w-fit mx-auto'>
						<div className='text-center inline-flex items-center gap-2 text-sm text-gray-500'>
							<InfoIcon className='w-6 h-6' />
							In case you device number health drops while running this campaign, you may send
							notification to selected numbers.
						</div>
						<div className='flex flex-col lg:flex-row gap-8'>
							<FormField
								...
								render={({ field }) => (
									<FormItem className='flex-1'>
										<FormLabel className='text-primary'>Health</FormLabel>
										<FormControl>
											<Select
												defaultValue='YELLOW'
												value={field.value}
												onValueChange={field.onChange}
											>
												<SelectTrigger className='h-8 text-black dark:text-white'>
													<SelectValue placeholder={'Number Health'} />
												</SelectTrigger>
												<SelectContent>
										...
										<FormControl>
											...
											>
												<Button variant={'outline'} className='w-full' size={'sm'}>
													Choose Numbers:-{' '}
													{field.value?.length ?? 0 ? `(${field.value.length})` : '(0)'}
												</Button>
											</NumberInputDialog>
										</FormControl>
									</FormItem>
							...
							<FormItem className='flex flex-col gap-2 flex-1'>
								...
								<TemplateDialog
									template={{
										template_id: fields.warning?.template_id ?? '',
										template_name: fields.warning?.template_name ?? '',
										buttons: fields.warning?.template_buttons,
										carousel: fields.warning?.template_carousel,
										header: fields.warning?.template_header,
										body: fields.warning?.template_body ?? [],
									}}
									onConfirm={(details) =>
										handleWarningTemplateSave(
											details as unknown as TemplateMessageWithoutForwardProps
										)
									}
								>
									<Button variant={'outline'} size='sm' className='w-full'>
										{fields.warning?.template_name
											? fields.warning?.template_name
											: 'Select Template'}
									</Button>
								</TemplateDialog>
					...
					<div className='flex justify-center gap-4'>
						<ForceScheduleDialog onConfirm={scheduleCampaign}>
							<Button className='w-[150px]' disabled={!isValid.success || loading}>
								Start
							</Button>
						</ForceScheduleDialog>
					</div>
