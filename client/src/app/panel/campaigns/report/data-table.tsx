...
export default function ReportDataTable() {
	const { list, setSearchText } = useBroadcast();
			...
			<Table>
				...
				<TableBody>
					<Each
						items={list}
						render={(recurring) => {
							const link = `/panel/campaigns/button-report/${recurring.broadcast_id}?name=${recurring.name}`;
							return (
								<TableRow>
									...
									<TableCellLink href={link}>{recurring.startDate}</TableCellLink>
