	...
	return (
		<div className='flex gap-3 items-end'>
			<div className='flex-1'>
				<Label>Section text ({sectionText.length}/24)</Label>
				<div className='flex-1'>
					<Input
						placeholder={placeholder}
						value={sectionText}
						onChange={(e) => setButtonText(e.target.value.substring(0, 24))}
					/>
				</div>
			</div>
			<div>
				<Button
					disabled={isDisabled || sectionText.length > 24}
					onClick={() => {
						if (!sectionText || sectionText.length > 24) return;
