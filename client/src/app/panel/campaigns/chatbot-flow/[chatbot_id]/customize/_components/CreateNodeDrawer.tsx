...
import { MdAudiotrack, MdContactPage, MdLabelImportant } from 'react-icons/md';
...
export default function CreateNodeDrawer({ addNode }: Props) {
	...
	const handleTemplateMessage = (
		details: TemplateMessageWithoutForwardProps,
		delay: number,
		reply_to_message: boolean
	) => {
		addNode({
	...
	return (
		<Sheet open={isOpen} onOpenChange={setSheetOpen}>
			...
			<SheetContent className='p-0'>
				<SheetHeader>
					<SheetTitle className='p-4 text-center'>Select One</SheetTitle>
				</SheetHeader>
				<div className=' px-8 h-full w-full overflow-y-auto'>
...
const MessageType = React.forwardRef<HTMLDivElement, MessageTypeProps>(
	({ body, icon, className, onClick = () => {} }: MessageTypeProps, ref) => {
		return (
			<div
				ref={ref}
				className={cn(
