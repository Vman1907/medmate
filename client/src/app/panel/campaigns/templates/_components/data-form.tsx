...
export default function DataForm({
	...
	const saveTemplate = async (data: Template, handle?: string) => {
		...
		if (!/^(?!_)[a-z0-9_]+$/.test(data.name)) {
			return toast.error(
				'Name should be lowercase without any spaces and first letter should be alphabet'
			);
		}
		onSave(data);
	};
	...
	function handleSave(data: Template) {
		if (header && header?.format !== 'NONE' && header?.format !== 'TEXT') {
			if (!file) {
										...
										<FormItem className='space-y-0 flex-1'>
											...
											<FormControl>
												<Select
													...
													onValueChange={(value) => {
														if (value === 'UTILITY') {
															form.setValue('header.format', 'NONE');
									...
									render={({ field }) => (
										<Show.ShowIf condition={!!form.getValues('header')}>
											<FormItem className='space-y-0 flex-1'>
												...
												<FormControl>
													<Select
														disabled={category === 'UTILITY'}
														value={category === 'UTILITY' ? 'NONE' : field.value?.format}
							...
							<Show.Else>
								...
								<div className='flex flex-col gap-3'>
									...
									<div className='flex flex-col md:flex-row gap-3'>
										...
										/>
										<URLButton
											disabled={(buttons ?? []).length >= 3 || (URLButtons ?? []).length === 1}
											onSubmit={addURLButton}
										/>
									</div>
								</div>
							</Show.Else>
