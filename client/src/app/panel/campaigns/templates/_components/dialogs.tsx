...
export function URLButton({
	...
	function handleWhatsappChatLink(link: string) {
		setLink(link);
	}
				...
				<div className='grid gap-3'>
					...
					<div className='grid gap-2'>
						<BodyWithVariables
							...
							infoText='Use dynamic variables in query params only'
						/>
					</div>
				</div>
				<AlertDialogFooter className='mt-2'>
					<div>
						<CreateWhatsappChatLink onSubmit={handleWhatsappChatLink} />
					</div>
...
function CreateWhatsappChatLink({ onSubmit }: { onSubmit: (link: string) => void }) {
	...
	return (
		<AlertDialog>
			<AlertDialogTrigger ref={buttonRef} asChild>
			...
			<AlertDialogContent className='sm:max-w-[425px] md:max-w-lg lg:max-w-xl'>
				<AlertDialogHeader>
					<AlertDialogTitle>Whatsapp Chat Link</AlertDialogTitle>
				</AlertDialogHeader>
				<div className='grid gap-3'>
					<div className='grid gap-2'>
						<Label className='text-primary' htmlFor='name'>
