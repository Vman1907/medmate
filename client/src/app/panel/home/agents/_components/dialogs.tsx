...
import { z } from 'zod';
import { updateAgentPermission } from '../action';
...
export function PermissionDialog({
	...
	const { handleSubmit, register, watch, setValue } = useForm<z.infer<typeof permissionsSchema>>({
	...
	async function formSubmit(values: z.infer<typeof permissionsSchema>) {
		...
		toast.promise(updateAgentPermission(id, values), {
			loading: 'Updating...',
			success: () => {
				onClose?.();
