...
export function AgentContextMenu({
	...
	return (
		<>
			<DropdownMenu>
				...
				<DropdownMenuContent className='w-64'>
					<DropdownMenuItem inset asChild>
						<Link href={`agents/${id}/logs`}>View Logs</Link>
					</DropdownMenuItem>
					...
					<DropdownMenuItem inset onClick={() => setPermissions(true)}>
						Permissions
					</DropdownMenuItem>
					<DropdownMenuSub>
			...
			{permissions && (
				<PermissionDialog id={id} data={agent.permissions} onClose={() => setPermissions(false)} />
			)}
		</>
	);
}
