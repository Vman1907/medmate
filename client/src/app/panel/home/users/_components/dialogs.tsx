...
import { addBalance, ExtendAdminExpiry, SetMarkUpPrice, UpgradePlan } from '../action';
...
export function AddBalanceDialog() {
	...
	const searchParams = useSearchParams();
	const pathName = usePathname();
	const id = searchParams.get('add-balance');
	...
	const handleSave = () => {
		...
		toast.promise(addBalance(id, Number(amount)), {
	...
	return (
		...
		>
			<DialogContent>
				...
				<DialogDescription>Balance will be added to the existing wallet balance.</DialogDescription>
				<Input
