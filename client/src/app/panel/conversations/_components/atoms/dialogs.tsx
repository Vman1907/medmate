...
import { AddButton, AddSections, ListButtons } from './add-ons';
...
export function QuickButtonTemplateMessage({
	...
	const { isAdmin, isMaster } = useUserDetails();
							...
							<Each
								...
								render={(template) => (
									<TableRow key={template.id}>
										...
										<Show.ShowIf condition={isAdmin || isMaster}>
											<TableCell>
												<DropdownMenu>
													<DropdownMenuTrigger>
...
export function QuickListTemplateMessage({
	...
	const handleAddQuickReply = (
		...
		toast.promise(promise, {
			loading: 'Adding quick list message...',
			success: (res) => {
				if (id) {
					updateQuickReply(res.id, res);
				} else {
				...
				return 'Quick list message saved successfully';
			},
			error: (err) => {
				return 'Failed to save quick list message';
			},
		});
		setSelectedReply('');
	};
...
export function AddQuickListMessage({
	...
	const handleSave = () => {
		...
		onConfirm(header, body, footer, sections, id);
		setHeader('');
		setBody('');
		setFooter('');
		setSections([]);
		...
		<Dialog>
			...
			<DialogContent className='sm:max-w-[425px] md:max-w-lg lg:max-w-2xl'>
				...
				<div className='max-h-[70vh] grid gap-2 overflow-y-auto px-0'>
					...
					<AddSections placeholder={'Create a new section. \nex. Menu 1'} onSubmit={addSections} />
				</div>
...
export function AddQuickFlowTemplateMessage({
	...
	const handleSave = () => {
		if (!body || !button_text || !flow_id) return toast.error('All fields are required');
