...
import AssignMessageLabelDialog from '../atoms/assign-message-label-dialog';
		...
		<div className={cn('w-full flex flex-row rounded-xl', isMe ? 'justify-start' : 'justify-end')}>
			...
			>
				<div className={cn('flex mb-1 gap-1 items-end', isMe ? 'flex-row' : 'flex-row-reverse')}>
					<div
						className={cn(
							'group flex flex-col  rounded-t-xl relative',
							!message.header_type && !message.body?.media_id ? 'pl-3 pr-4 py-1.5' : 'px-1 py-1',
							isMe
				...
				>
					...
					{message.failed_at && <FormatTime time={message.failed_at} />}
					{message.read_at ? (
