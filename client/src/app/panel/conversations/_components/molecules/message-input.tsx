...
import QuickReplyDialog from '../atoms/add-quick-reply-dialog';
import {
...
export type TemplateMessageWithoutForwardProps = {
	template_id: string;
...
export default function MessageBox({ isExpired }: { isExpired: boolean }) {
	...
	const { active_recipient: selected_recipient } = useConversationStore();
	...
	function sendQuickTemplateMessage(details: TemplateMessageWithoutForwardProps) {
		MessagesService.sendQuickTemplateMessage({
			...details,
			recipientId: selected_recipient!.id,
			context: { message_id: replyMessageId },
			...
			<div className='flex bg-white w-full p-2 items-end gap-1'>
				<DropdownMenu>
					...
					<DropdownMenuContent className='w-56'>
						...
						<TemplateDialog
							onConfirm={(details) =>
								sendQuickTemplateMessage(details as unknown as TemplateMessageWithoutForwardProps)
							}
						>
