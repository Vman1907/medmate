...
export default class ContactService {
	static async listContacts() {
		try {
			const data = await apiClient.get<{ contacts: any[] }>(`/contacts?page=1&limit=100000`, {
				tags: [REVALIDATE_TAGS.CONTACTS],
			});
	...
	static async addContact(contact: Omit<Contact, 'id' | 'formatted_name'>) {
		const data = await apiClient.post<{ contact: ContactWithID }>(`/contacts`, contact);
		await apiClient.revalidateTag(REVALIDATE_TAGS.CONTACTS);
	...
	static async updateContact(
		...
		const data = await apiClient.put<{ contact: ContactWithID }>(
			`/contacts/${contact.id}`,
			contact
		);
		await apiClient.revalidateTag(REVALIDATE_TAGS.CONTACTS);
